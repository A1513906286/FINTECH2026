...
def home():
    ...
    bill_data = {
        "last_spend": f"{bill_row['amount']:.2f}" if bill_row else "0.00",
        "last_spend_currency": bill_row['currency'] if bill_row else "USD",
        "converted_spend": f"{bill_row['converted_amount']:.2f}" if bill_row else "0.00"
    }
...
def upload_bank_statement():
    ...
    try:
        ...
        # 🔧 同时保存余额信息（一个PDF包含流水和余额）
        balance = result.get('current_balance') or result.get('balance', 0)
        registration_temp_data[session_id]['balance'] = balance
        registration_temp_data[session_id]['currency'] = 'CNY'
        ...
        # 存储完整的PDF数据（用于增强预测）
        registration_temp_data[session_id]['pdf_data'] = result
        ...
        return jsonify({
            ...
            'data': {
                ...
                'current_balance': result.get('current_balance', result.get('balance', 0)),
                'total_transactions': result['total_transactions'],
                'formatted_transactions': result.get('formatted_transactions', [])[:10],  # 只返回前10笔给前端预览
                'statistics': {
                    'avg_income_3m': result.get('avg_income_3m', 0),
                    'avg_monthly_consumption': result.get('avg_monthly_consumption', 0),
                    'min_balance_12m': result.get('min_balance_12m', 0),
                    'large_withdrawal_ratio': result.get('large_withdrawal_ratio', 0)
                }
            }
        })
...
def predict_credit_limit():
    """
    预测信用额度（增强版：使用完整的PDF数据和Home Credit模型）
    """
    try:
        # 从临时数据中获取信息
        session_id = session.get('temp_registration_id')
        ...
        if not session_id or session_id not in registration_temp_data:
            ...
            # 获取PDF详细数据（如果有）
            pdf_data = temp_data.get('pdf_data', None)

            # 优先从pdf_data中获取数据
            if pdf_data:
                total_income = pdf_data.get('total_income', temp_data.get('total_income', 74707.66))

                # 调试：查看pdf_data中的余额字段
                pdf_current_balance = pdf_data.get('current_balance')
                pdf_balance = pdf_data.get('balance')
                temp_balance = temp_data.get('balance')

                print(f"[调试] pdf_data.current_balance: {pdf_current_balance}")
                ...
                print(f"✓ 从临时数据获取: 收入=¥{total_income:,.2f}, 余额=¥{balance:,.2f}")
        ...
        if result['success']:
            ...
            if session_id and session_id in registration_temp_data:
                ...
                registration_temp_data[session_id]['risk_level'] = result.get('risk_level', '未知')
                registration_temp_data[session_id]['pd_prob'] = result.get('pd_prob', 0)
